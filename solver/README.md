...
`src/bin/test_request_2.rs` is a second scenario built the same way (`cargo run --bin test_request_2`): 8 identical 75mm cubes into one 150mm SML carton, which fills it exactly as a 2×2×2 stack. Where `test_request.rs`'s items all land in a single flat layer (z stays 0), this one exercises genuine vertical stacking. Rough isometric diagrams of both scenarios' packing results are in `docs/misc/` (`med_carton_packing.html`, `sml_carton_packing.html`) — hand-derived from the actual solver output, not live-rendered, so they need to be regenerated by hand if the test data changes.
